...
describe('AuthService', () => {
  ...
  describe('SignIn', () => {
    ...
    it('should throw user not found error', () => {  
      ...
      expect(service.signIn(username, password)).rejects.toThrow(new UnauthorizedException('User not found'))
    });
    ...
    it('should throw password is incorrect error', () => {
      ...
      expect(service.signIn(username, password)).rejects.toThrow(new UnauthorizedException('Password is incorrect'))
    });
    ...
    it('should return accesstoken with username not including artistId in the payload', async () => {
      ...
      expect(mockArtistService.getArtistByUserId).toHaveBeenCalledWith(mockUser.id);
